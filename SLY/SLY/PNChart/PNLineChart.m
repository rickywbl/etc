...
- (void)setXLabels:(NSArray *)xLabels withWidth:(CGFloat)width {
    ...
    if (_showLabel) {
        for (int index = 0; index < xLabels.count; index++) {
            ...
            NSInteger x = (index * _xLabelWidth + _chartMarginLeft + _xLabelWidth / 2.0 - _xLabelWidth/2);
...
- (NSString *)formatYLabel:(double)value {
    ...
    if (self.yLabelBlockFormatter) {
        ...
        if (!self.thousandsSeparator) {
            NSString *format = self.yLabelFormat ?: @"%.2f";
            
            NSString * path = [NSString stringWithFormat:format, value];
        ...
        NSString * path = [numberFormatter stringFromNumber:[NSNumber numberWithDouble:value]];
